...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        if self.is_mec :
            ...
            # loc, time_window, resource, demand, cpu_cl
            node_dim = 7
    ...
    def forward(self, input, selections, return_pi=False):
        """
        ...
        _log_p, pi, state_cost, state_tw = self._inner(input, embeddings, selections)       
        cost, mask = self.problem.get_costs(input, pi, selections, state_cost, state_tw)
    ...
    def _calc_log_likelihood(self, _log_p, a, mask):
       ...
       # assert (log_p_node> -1000).data.all(),"Logprobs should not be -inf, check sampling procedure!"
        ...
        return log_p.sum(1)
        #log_p_node.sum(1)
    def normal_data(self, raw_data):
        # 创建一个 MinMaxScaler 实例
        data = raw_data.to('cpu').numpy()
        scaler = MinMaxScaler()
        data_normalized = np.empty_like(data)

        # 对每个通道分别进行归一化
        for i in range(data.shape[2]):  # 遍历每个坐标点
            data_normalized[:, :, i] = scaler.fit_transform(data[:, :, i])
        return torch.from_numpy(data_normalized).to(raw_data.device)

    def _init_embed(self, input):

        if self.is_mec:
            task_info = torch.cat((input["task_position"], input["time_window"]/(10), input["IoT_resource"],input["task_data"],input["CPU_circles"]), dim=-1)
    ...
    def _inner(self, input, embeddings, selected_indices):
