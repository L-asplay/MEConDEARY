...
class WarmupBaseline(Baseline):
    ...
    def epoch_callback(self, Selectlayer, model, epoch):
        # Need to call epoch callback of inner model (also after first epoch if we have not used it)
        self.baseline.epoch_callback(Selectlayer, model, epoch)
...
class RolloutBaseline(Baseline):
    ...
    def __init__(self, Selectlayer, model, problem, opts, epoch=0):
        super(Baseline, self).__init__()

        self.problem = problem
        self.opts = opts

        self._update_model(Selectlayer, model, epoch)

    def _update_model(self, Selectlayer, model, epoch, dataset=None):
        self.Selectlayer = copy.deepcopy(Selectlayer)
        ...
        if dataset is None:
            self.dataset = self.problem.make_dataset(dependency = self.opts.priority,
                size=self.opts.graph_size, num_samples=self.opts.val_size, distribution=self.opts.data_distribution)
        else:
            self.dataset = dataset
        print("Evaluating baseline model on evaluation dataset")
        self.bl_vals = rollout(self.Selectlayer, self.model, self.dataset, self.opts).cpu().numpy()
    ...
    def wrap_dataset(self, dataset):
        ...
        return BaselineDataset(dataset, rollout(self.Selectlayer, self.model, dataset, self.opts).view(-1, 1))
    ...
    def eval(self, x, c):
        ...
        with torch.no_grad():
            v, _, _ = EvalForRam(self.Selectlayer, self.model, x)  
    ...
    def epoch_callback(self, Selectlayer, model, epoch):
        ...
        candidate_vals = rollout(Selectlayer, model, self.dataset, self.opts).cpu().numpy()
    ...
    def load_state_dict(self, state_dict):
        ...
        load_selector = copy.deepcopy(self.Selectlayer)
        get_inner_model(load_model).load_state_dict(get_inner_model(state_dict['model']).state_dict())
        get_inner_model(load_selector).load_state_dict(get_inner_model(state_dict['Selectlayer']).state_dict())
        self._update_model(load_selector, load_model, state_dict['epoch'], state_dict['dataset'])
