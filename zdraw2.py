...
def plot_box_with_mean(data_dict, pic='./za/e.png',title=None,):
    ...
    plt.ylim(list(data_dict[labels[1]].values())[3], list(data_dict[labels[5]].values())[0])  # 纵轴范围
    plt.rcParams['ytick.labelsize'] = 16
    plt.xlabel("Number of offloading tasks", fontsize=27)  # 横轴标签
    plt.ylabel("Total energy", fontsize=27)  # 纵轴标签
    # 设置图例（关键参数）
    plt.legend(
      bbox_to_anchor=(0.99, 0.99),  # 锚点在图内右侧边缘（x=0.9，y=1）
      loc='upper right',         # 图例左上角对齐锚点，放入图内
      borderaxespad=0.01,        # 图例与坐标轴的间距（可调整）
      fontsize=18,              # 图例文本字体大小
...
def runtest(opts):
    ...
    size=[40,40,40,40]
    path=['./zzzz/mec40/4/epoch-49.pt',
          './zzzz/mec40/8/epoch-49.pt',
          './zzzz/mec40/12/epoch-49.pt',
          './zzzz/mec40/16/epoch-49.pt'
    ]
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--cost', default='./za/d.npz')
    ...
    # for i in range(6):
    #   cost_dict_i[i] = {
    #     "4" : np.array(costlist[0][i]),
    #     "8" : np.array(costlist[1][i]),
    #     "12" : np.array(costlist[2][i]),
    #     "16" : np.array(costlist[3][i]),
    #   }
    ...
    data = dict(np.load("./za/d.npz", allow_pickle=True))
    a = {}
...
# nohup python -u zdraw2.py  >./za/f.log 2>&1 &
