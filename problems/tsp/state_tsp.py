...
class StateTSP(NamedTuple):
    ...
    def get_final_cost(self):
        ...
        return self.lengths + (self.loc[self.ids, self.first_a, :] - self.cur_coord).norm(p=2, dim=-1)/self.speed*self.P_fly
    ...
    def update(self, selected):
        ...
        if self.cur_coord is not None:  # Don't add length for first action (selection of start node)
            lengths = self.lengths + (cur_coord - self.cur_coord).norm(p=2, dim=-1)/self.speed*self.P_fly  # (batch_dim, 1)
